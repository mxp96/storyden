...
func (m *Manager) Add(ctx context.Context, accountID account.AccountID, address mail.Address) (*account.EmailAddress, error) {
	...
	if err != nil {
		// Check if error is because email already exists and is verified
		if ftag.Get(err) == ftag.AlreadyExists {
			return nil, fault.Wrap(err, 
				...
				fmsg.WithDesc(
					...
					"This email address is already added to your account and verified. You cannot add the same email twice.",
				),
			)
		}
		return nil, fault.Wrap(err, fctx.With(ctx))
